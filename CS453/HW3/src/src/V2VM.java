...
public class V2VM {
    public static void main(String[] args) {
        ...
        for (CFG cfg : cfgs) {
            RegisterAllocator reg_alloc = new RegisterAllocator(cfg.intervals, cfg.vars);
            reg_alloc.LinearScanRegisterAllocation();
            cfg.used_regs = reg_alloc.used_regs;
            cfg.spill_stack_size = reg_alloc.spill_stack;
            if (debug)
        ...
        for (CFG cfg : cfgs) {
            ...
            vaporm.append("local " + (cfg.calls_func ?
                    cfg.used_regs.size() + cfg.spill_stack_size :
                    cfg.spill_stack_size) + "]\n");
            ...
            for (Variable var : cfg.vars) {
                if (var.arg != null) {
                    if (var.arg instanceof StackedRegister) {
                        if (!var.intervals.isEmpty()) { // variable in use
                            vaporm.append(Element.tab + var.intervals.get(0).register +
                                    " = in[" + stack_index++ + "]\n");
                        } else { // variable not in use
                            stack_index++;
                        }
                    } else {
                        if (!var.intervals.isEmpty()) { // variable in use
